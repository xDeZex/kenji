...
**Update (2026-08-05, first real test — fika/steven trial):** the gating held (conclusions did route through the user before reaching Steven), but the dialogue itself was too shallow — one message laying out two fully-drafted countermeasures, one reply reacting to both. The user called this out directly and asked for deeper dialogue, modeled on how a real Toyota Kata coaching cycle runs: a loop of gated micro-turns on one obstacle at a time, not a single dump-and-react message. See the `coaching-kata` skill for the resulting dialogue mechanics — this memory note stays the record of *why* gating exists; that skill is the *how* of running the exchange itself, now standard work rather than freeform judgment each time.
